...
// Navigation Component - Maintains exact UI theme
const Nav = ({ logs, isStarred, setIsStarred }) => {
  ...
  return (
    <nav className="bg-[#86b059] text-white px-6 py-2 flex justify-between items-center shadow-md sticky top-0 z-50">
      <div className="flex items-center gap-4">
        <Link to="/" className="font-bold italic text-lg tracking-tighter">
      ...
      <div className="flex items-center gap-5">
        ...
        {/* FUNCTIONAL STAR TOGGLE - Linked to Watchlist */}
        <button 
          onClick={() => setIsStarred(!isStarred)} 
          className={`transition-all duration-300 ${isStarred ? 'text-yellow-300 scale-110' : 'hover:opacity-80'}`}
          title={isStarred ? "Showing Watchlist" : "Show All Users"}
        >
        ...
        <div className="relative" ref={notifRef}>
          <button onClick={() => setShowNotifs(!showNotifs)} className="relative hover:opacity-80">
            ...
            {criticalAlerts.length > 0 && (
              <span className="absolute -top-1 -right-1 bg-red-500 text-[8px] w-4 h-4 flex items-center justify-center rounded-full border border-[#86b059]">
          ...
          {showNotifs && (
            <div className="absolute right-0 mt-4 w-72 bg-white shadow-2xl border border-gray-200 rounded-md text-slate-800 z-50 animate-in fade-in slide-in-from-top-2">
              ...
              <div className="max-h-64 overflow-y-auto">
                {criticalAlerts.length === 0 ? (
                  <p className="p-6 text-center text-xs text-gray-400 italic">No threats</p>
                ) : (
            ...
            <div className="absolute right-0 mt-4 w-56 bg-white shadow-xl border border-gray-200 rounded p-4 text-slate-800 z-50 animate-in fade-in slide-in-from-top-2">
              ...
              <div className="space-y-2">
                <div className="flex justify-between text-[11px]">
                  <span>MongoDB</span>
                  <span className="text-green-500 font-bold font-mono text-[10px]">ONLINE</span>
                </div>
                <div className="flex justify-between text-[11px]">
                  <span>FastAPI</span>
                  <span className="text-green-500 font-bold font-mono text-[10px]">PORT 8000</span>
                </div>
              </div>
            </div>
          )}
...
export default function App() {
  ...
  // Filtering Logic combining Time + Watchlist
  useEffect(() => {
    let filtered = [...allLogs];

    // 1. Time timeframe filter (Last Day / Create Date)
    if (timeframe === 'last_day') {
      const oneDayAgo = new Date(Date.now() - 24 * 60 * 60 * 1000);
      filtered = filtered.filter(log => new Date(log.timestamp) >= oneDayAgo);
    }

    // 2. Star filter (Watchlist) - Only show logs from favorited users
    if (isStarred) {
      filtered = filtered.filter(log => favoritedUsers.includes(log.username));
    }

    setDisplayLogs(filtered);
  }, [timeframe, allLogs, isStarred, favoritedUsers]);

  const fetchData = async () => {
    try {
      const [logsRes, agentsRes] = await Promise.all([
        threatService.getLogs(), 
        threatService.getAgentStatus()
      ]);
  ...
  if (loading) return (
    <div className="flex h-screen items-center justify-center bg-[#f4f7f6] text-[10px] font-black uppercase tracking-widest text-gray-400">
      Syncing SIEM Database...
    </div>
  );
  ...
  return (
    <BrowserRouter>
      <div className="min-h-screen bg-[#f4f7f6]">
        {/* Pass Star State to Nav */}
        <Nav logs={allLogs} isStarred={isStarred} setIsStarred={setIsStarred} />
